...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        identity = torch.eye(self.num_ways)
        identity.requires_grad = False
        ground_truths = (identity[labels]).to(torch.float32)

        # inner loop updates
        for j in range(self._num_inner_steps):
            # compute the accuracy with current parameter
            logits = self._forward(images,parameters)
            acc = util.score(logits,labels)
            accuracies.append(acc)
            
            # perform the update step
            loss = F.cross_entropy(logits,ground_truths.to(DEVICE))

            gradients = autograd.grad(loss,[v for k,v in parameters.items()],create_graph=train)
            keys = [k for k,v in  parameters.items()]
            for i in range(len(keys)):
                k = keys[i]
                parameters[k] = parameters[k].clone() - self._inner_lrs[k]*gradients[i]
                # in above clone used to keep the complier happy from:
                # RuntimeError: one of the variables needed for gradient computation has been modified by an inplace operation
        
        # compute the accuracy for the final parameter
        logits = self._forward(images,parameters)
        acc = util.score(logits,labels)
        accuracies.append(acc)
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            # compute adapted parameters via the inner loop
            ...
            ground_truths = (identity[labels_query]).to(torch.float32)
            logits = self._forward(images_query,parameters)
            loss = F.cross_entropy(logits,ground_truths.to(DEVICE))
            outer_loss_batch.append(loss)
            
            # compute query set accuracy for this task
            acc = util.score(logits,labels_query)
