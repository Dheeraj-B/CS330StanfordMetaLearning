...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            num_support = 0
            i = 0
            while labels_support[i] == 0:
                num_support+=1
                i+=1
            
            num_ways = len(labels_support)//num_support
            
            # compute the representations of the support images
            support_latents = self._network(images_support)
            prototypes = []
            for i in range(num_ways):
                # forcing the support labels to be in the order 0,..,num_ways-1
                assert labels_support[i*num_support] == i
                
                tot = 0
                for j in range(num_support):
                    tot+=support_latents[i*num_support+j]
                prototypes.append(tot/num_support)
            
            query_latents = self._network(images_query)
            # convert prototypes to tensor
            prototypes = torch.stack(prototypes)
            
            # for each image in the query, calculate its distance to each prototype
            assert prototypes.shape[-1] == query_latents.shape[-1]
            
            # unsqueeze for broadcasting
            query_reshaped = query_latents.unsqueeze(1) # N by 1 by d
            proto_reshaped = prototypes.unsqueeze(0) # 1 by num_prototypes by d
            distance = ((query_reshaped - proto_reshaped)**2).sum(dim=-1) # N by num_prototypes
            
            # compute the softmax
            logits = F.softmax(-distance,dim=1) # N by num_prototypes
            
            # compute the ground truth labels
            identity = torch.eye(num_ways)
            ground_truths = (identity[labels_query]).to(torch.float32) # N by num_ways
            
            loss = F.cross_entropy(logits,ground_truths.to(DEVICE))
            loss_batch.append(loss)
            
            # compute accuracies
            acc_query = util.score(logits,labels_query)
            accuracy_query_batch.append(acc_query)
            
            support_reshaped = support_latents.unsqueeze(1)
            distance_supp = ((support_reshaped - proto_reshaped)**2).sum(dim=-1)
            logits_supp = F.softmax(-distance_supp,dim=1)
            acc_supp = util.score(logits_supp,labels_support)
