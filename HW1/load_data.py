...
class DataGenerator(IterableDataset):
    ...
    def __init__(
        ...
        self.label_nums = [i for i in range(num_classes)]
        self.eye = np.eye(num_classes,dtype=int)
    ...
    def image_file_to_array(self, filename, dim_input):
        """python hw1.py --num shot K --num classes N
    ...
    def flatten_and_pair(self,arr,shuffle=False):
        #arr is a 2d numpy array of the form [['1','path1'],['2','path2']]
        
        # shuffle required for the query set
        if shuffle:
            np.random.shuffle(arr)
        
        labels = np.array(arr[:,0],dtype=int)
        paths = arr[:,-1]
        
        flat = [self.image_file_to_array(file,784) for file in paths]
        one_hot_labels = [list(self.eye[lab]) for lab in labels]
        
        
        return flat, one_hot_labels
    ...
    def _sample(self):
        ...
        # first sample N classes
        select_classes = random.sample(self.folders, self.num_classes)
        # then give the selected classes and a list of labels to get_images to process
        x = get_images(select_classes, self.label_nums, nb_samples = self.num_samples_per_class, shuffle=False)
        x = np.array(x)
        
        x = x.reshape(len(self.label_nums),self.num_samples_per_class,2)
        x = np.swapaxes(x,0,1)
        
        #at this point x looks like: [[[1,'path'],[2,path]],[[1,'path'],[2,path]],[[1,'path'],[2,path]]] (k=3)
        imgs = []
        labels = []
        
        
        for i in range(self.num_samples_per_class-1):
            flat, lab = self.flatten_and_pair(x[i],shuffle=False)
            
            imgs.append(flat)
            labels.append(lab)
            
            
        # process the last query set and shuffle the order
        flat, lab = self.flatten_and_pair(x[-1],shuffle=True)
        
        imgs.append(flat)
        labels.append(lab)
        
        return np.array(imgs).astype(np.float32), np.array(labels).astype(np.float32)
