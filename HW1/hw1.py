...
class MANN(nn.Module):
    ...
    def forward(self, input_images, input_labels):
        ...
        B, Kplus, N, d = input_images.shape
        
        feed_labels = torch.cat([input_labels[:,:-1,:,:],torch.zeros_like(input_labels[:,-1,:,:]).view(B,1,N,N)],dim=1)
        
        
        # concatenating the falttened image and its one hot label
        z = torch.cat([input_images,feed_labels],dim=-1)
         
        B, Kplus, N, d = z.shape
        
        z = z.view(B,(Kplus)*N,d)
        
        x, _ = self.layer1(z)
        preds,_ = self.layer2(x)
        
        preds = preds.view(B,Kplus,N,N)
    ...
    def loss_function(self, preds, labels):
        ...
        return F.cross_entropy(pred_labels,true_labels)
        #return F.cross_entropy(pred_labels,true_labels.float())
