...
def get_icl_prompts(
    ...
    **Note**: Be sure to shuffle the support examples and labels 
    ...
    if len(support_inputs) == 0:
        if prompt_mode == 'qa':
            return test_input + ' In the'
        elif prompt_mode == 'none':
            return test_input
        elif prompt_mode == 'tldr':
            return test_input+' TL;DR:'
        else:
            return 'Context: '+test_input+' One sentence summary:'
    
    # shuffle while preserving the order
    c = list(zip(support_inputs, support_labels))
    random.shuffle(c)
    questions, answers = zip(*c)
    
    if prompt_mode == 'qa':
        # combine all the in-context examples to the prompt
        prompt = ' '.join([x + ' In the '+ y +'.' for x, y in zip(questions, answers)])
        
        # add the final question to the prompt without space at the end.
        prompt+=' '+test_input + ' In the'
        
    elif prompt_mode == 'none':
        prompt = ' '.join([questions[i] + ' ' + answers[i] for i in range(len(questions))])
        
        prompt += ' '+test_input
    
    elif prompt_mode == 'tldr':
        prompt = ' '.join(x+' TL;DR: '+y for x,y in zip(questions,answers))
        
        prompt+=' '+test_input+' TL;DR:'
            
    else: # custom prompt
        # I try annotating the context and one line summary explcitly
        prompt = ' '.join('Context: '+ x + ' One sentence summary: '+y for x,y in zip(questions,answers))
        
        prompt+=' Context: '+test_input+' One sentence summary:'
...
def do_sample(model, input_ids, stop_tokens, max_tokens):
    ...
    while num_tokens < max_tokens:
        ...
        if idx.item() in stop_tokens:
            break
        
        prefix = torch.cat((prefix,idx.view(1,1)),-1)
        
    return prefix.squeeze().tolist()[init_len:] 
                ...
                for k in ks:
                    ...
                    for repeat in range(args.repeats):
                        ...
                        for row in pbar:
                            ...
                            output_ids = do_sample(model,input_ids,stop_tokens,max_tokens)
                            #print(prompt_mode)
                            #print(prompt)
                            ...
                            #print(max_tokens)
                            decoded_prediction =  tokenizer.decode(output_ids)[1:] # trimming the extra space at the beginning
                            #print(targets[row])
